...
class Player(sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.bolt_anim_left = animation.Animation(bolt_anim)
        self.bolt_anim_left.play()
        self.bolt_anim_left_super_speed = animation.Animation(bolt_anim_super_speed)
        self.bolt_anim_left_super_speed.play()

        self.bolt_anim_right = animation.Animation([(anim, ANIMATION_DELAY) for anim in ANIMATION_RIGHT])
        self.bolt_anim_right.play()
        self.bolt_anim_right_super_speed = animation.Animation([(anim, ANIMATION_SUPER_SPEED_DELAY) for anim in
                                                                 ANIMATION_RIGHT])
        self.bolt_anim_right_super_speed.play()

        self.bolt_anim_stay = animation.Animation(ANIMATION_STAY)
        self.bolt_anim_stay.play()

        self.bolt_anim_stay.blit(self.image, SCREEN_START)

        self.bolt_anim_jump_left = animation.Animation(ANIMATION_JUMP_LEFT)
        self.bolt_anim_jump_left.play()

        self.bolt_anim_jump_right = animation.Animation(ANIMATION_JUMP_RIGHT)
        self.bolt_anim_jump_right.play()

        self.bolt_anim_jump = animation.Animation(ANIMATION_JUMP)
